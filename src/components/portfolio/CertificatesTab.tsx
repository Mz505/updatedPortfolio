    ...
    <div>
      ...
      <div className="flex justify-center mb-6 sm:mb-8">
        <select
          ...
          className="bg-gray-800 border border-gray-600 rounded-lg px-3 sm:px-4 py-2 text-sm sm:text-base text-gray-300 focus:outline-none focus:border-purple-500"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        {sortedCertificates.map((cert, index) => (
          <div
            key={cert.id}
            className="group bg-gray-800/50 backdrop-blur-sm p-4 sm:p-6 rounded-2xl border border-gray-700/50 hover:border-purple-500/50 transition-all duration-500 sm:hover:scale-105 hover:shadow-2xl hover:shadow-purple-500/20 animate-slide-up"
            style={{ animationDelay: `${index * 150}ms` }}
          >
            {/* Certificate Header */}
            <div className="flex items-start justify-between mb-3 sm:mb-4">
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-purple-500 to-pink-500 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
            ...
            <div className="space-y-2 sm:space-y-3">
              <h3 className="text-base sm:text-lg font-bold text-white group-hover:text-purple-400 transition-colors duration-300">
                {cert.title}
              </h3>
              ...
              {/* Verify Button */}
              <button className="w-full mt-3 sm:mt-4 px-3 sm:px-4 py-2 sm:py-3 bg-gradient-to-r from-purple-600 to-pink-600 rounded-lg text-white text-sm sm:text-base font-semibold hover:from-purple-700 hover:to-pink-700 transform hover:scale-105 transition-all duration-300">
                Verify Certificate
              </button>
            </div>
          </div>
